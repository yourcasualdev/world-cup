...
export default function MatchCard({ match, index, teamFormMap = {} }: MatchProps) {
  ...
  return (
    <motion.div
      ...
      className={`group relative w-full flex flex-col md:flex-row border border-white/10 bg-pitch-black-light hover:border-neon-green/50 transition-all overflow-hidden ${isLive ? 'shadow-[0_0_20px_rgba(204,255,0,0.05)] border-neon-green/20' : 'hover:shadow-[0_0_30px_rgba(204,255,0,0.1)]'}`}
    >
      ...
      {/* Time & Status Column */}
      <div className="w-full md:w-32 bg-white/5 px-4 py-3 md:p-6 flex flex-row md:flex-col justify-between md:justify-start items-center md:items-start gap-3 shrink-0 border-b md:border-b-0 md:border-r border-white/10 relative z-10">
        <div className={`font-display text-2xl md:text-3xl transition-colors ${isLive ? 'text-neon-green' : 'text-white/60 group-hover:text-white'}`}>{timeString}</div>
      ...
      {/* Teams Column */}
      <div className="flex-1 flex flex-col justify-center relative px-4 py-5 md:px-12 z-10 overflow-hidden min-w-0">
        {/* Home */}
        <div className="flex items-center justify-between mb-4 relative">
          <Link href={`/teams/${match.homeTeam.id}`} className="flex items-center gap-4 group/team">
            ...
            <span className={`font-display text-3xl sm:text-4xl lg:text-6xl uppercase tracking-tight transition-all duration-300 group-hover/team:text-neon-green group-hover/team:translate-x-2 ${homeWon ? 'text-stark-white' : 'text-white/70'}`}>
              {match.homeTeam.tla}
            </span>
          </Link>
          <div className={`font-display text-3xl sm:text-4xl lg:text-6xl ${homeWon ? 'text-neon-green' : 'text-stark-white'} ${isLive ? 'animate-pulse' : ''}`}>
            {match.homeScore ?? "-"}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between relative">
          <Link href={`/teams/${match.awayTeam.id}`} className="flex items-center gap-4 group/team">
            ...
            <span className={`font-display text-3xl sm:text-4xl lg:text-6xl uppercase tracking-tight transition-all duration-300 group-hover/team:text-neon-green group-hover/team:translate-x-2 ${awayWon ? 'text-stark-white' : 'text-white/70'}`}>
              {match.awayTeam.tla}
            </span>
          </Link>
          <div className={`font-display text-3xl sm:text-4xl lg:text-6xl ${awayWon ? 'text-neon-green' : 'text-stark-white'} ${isLive ? 'animate-pulse' : ''}`}>
            {match.awayScore ?? "-"}
          </div>
        </div>
      </div>
      ...
      {/* Stage Info Column */}
      <div className="w-full md:w-48 px-4 py-2 md:p-6 bg-white/5 border-t md:border-t-0 md:border-l border-white/10 flex flex-row md:flex-col justify-between items-center md:items-end shrink-0 relative z-10">
        <span className="font-body text-[9px] md:text-[10px] font-bold tracking-widest text-white/40 uppercase md:text-right leading-relaxed">
          {match.stage.replace(/_/g, ' ')}
        </span>
        <span className="font-body text-[9px] font-bold tracking-widest text-white/20 uppercase">
          #{match.id.slice(-4)}
        </span>
      </div>
    </motion.div>
